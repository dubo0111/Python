...
def two_stage(ni,nk,rnd,a1):
    data = dg0.data_gen(ni,nk,rnd)
    cd,cdk,sk,coordinate = data.illustrative()
    p=2
    ...
    #    u = m.addVars(nk,ni,vtype=GRB.CONTINUOUS, name="u")
        L3 = m.addVars(nk,vtype=GRB.CONTINUOUS, obj=0.00001, name="L3")
    ...
    print('================= Two Stage Problem SOLUTION==================')
    print('1st stage:',L.x)
    ...
    for k in range(nk):
        for i in range(ni):
            ...
            value_u[k][i] = m.getVarByName(u_name).x
    value_v =[[[0 for j in range(ni)] for i in range(ni)] for k in range(nk)]
    for k in range(nk):
        for i in range(ni):
            for j in range(ni):
                v_name = ''.join(['v[',str(k),',',str(i),',',str(j),']'])
                value_v[k][i][j]= m.getVarByName(v_name).x
    value_L3 = []
    for k in range(nk):
        L3_name = ''.join(['L3[',str(k),']'])
        value_L3.append(m.getVarByName(L3_name).x)

    return L.x,eta.x,cd,value_y,value_x,value_u,value_v,value_L3,coordinate
    # for i in value_u:
    #     if i.x != 0:
    #         print(i.x)
